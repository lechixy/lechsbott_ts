...
export default new SlashCommand({
    ...
    async execute({ client, interaction, args }) {
        ...
        let searchresult = new Discord.Embed()
            ...
            .addField({name: 'Choose one of them', value: 'Just type the number one of results you wanted to play in 30s,\n Also for cancel you can type \`cancel\`'})
            ...
            .then(async responde => {
                ...
                if (['cancel', 'quit'].includes(collected.content)) {
                    let embed = new Discord.Embed()
                        .setColor(Discord.Util.resolveColor(roleColor(interaction)))
                        .setAuthor({name: `Cancelled from search`, iconURL: interaction.member.displayAvatarURL()})
